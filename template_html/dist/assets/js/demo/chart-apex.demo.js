...
var handleRenderApexChart = function() {
	...
	var apexPieChartOptions = {
		...
		colors: [ hexToRgba(app.color.pink, .75),  hexToRgba(app.color.orange, .75),  hexToRgba(app.color.theme, .75), 'rgba('+ app.color.whiteRgb + ', .5)',  hexToRgba(app.color.indigo, .75)],
	...
	var apexRadialBarChartOptions = {
		...
		colors: [COLOR_CYAN, app.color.theme, app.color.indigo, app.color.gray300],
		series: [76, 67, 61, 90],
		...
		plotOptions: {
			radar: {
				...
				polygons: {
					...
					fill: {
						colors: [hexToRgba(app.color.gray300, .25), 'rgba('+ app.color.whiteRgb + ', .25)']
					}
				}
			}
		},
